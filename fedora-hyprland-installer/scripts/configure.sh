...
GPU_ENVS=""
if command -v lspci &>/dev/null && lspci -nnk | grep -iq "nvidia"; then
...
# 4. Generate hyprland.conf if missing or if requested
if [ ! -f "${HYPR_CONF}" ]; then
    echo "[+] Creating clean ${HYPR_CONF}"
    cat <<EOF > "${HYPR_CONF}"
# Fedora Hyprland Default Configuration
# Generated by Antigravity Fedora Hyprland Installer Skill
...
exec-once = /usr/libexec/polkit-gnome-authentication-agent-1 || /usr/lib/polkit-gnome/polkit-gnome-authentication-agent-1
exec-once = waybar &
exec-once = dunst &
